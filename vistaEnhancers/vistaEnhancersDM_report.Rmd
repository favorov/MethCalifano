...
print.data.frame(karyotype.stat[DM.cytobands,],row.names=FALSE)
```

### Here is the same list with Benjamini-Hoochberg correction
```{r print, with FDR} 
print.data.frame(karyotype.stat[DM.cytobands.fdr,c(columns,'p.value','fdr','is.hyper')],row.names=FALSE)
```

### And, here is the same list with Bonferroni correction
```{r print, with Bonferroni} 
print.data.frame(karyotype.stat[DM.cytobands.bonferroni,c(columns,'p.value','bonferroni','is.hyper')],row.names=FALSE)
```
